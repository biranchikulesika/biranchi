...
import { getFragments, getJournalMoments, getPosts } from '@/app/admin/actions';
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (pData) {
          let visiblePosts = pData.filter((p:any) => !p.hidden && !p.draft && p.persona?.toLowerCase() === 'wanderer');
