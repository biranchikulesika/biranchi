        ...
        <AnimatePresence>
          {isOpen && query.length > 0 && (
            <motion.div
              ...
              className={`absolute top-full right-0 mt-4 w-[300px] backdrop-blur-xl p-2 shadow-2xl flex flex-col gap-1 z-50 rounded-lg ${mobileBgColor} border border-border text-foreground`}
            >
              {results.map((r, i) => (
                <div key={i} className={`px-3 py-2 text-sm rounded-md cursor-pointer transition-colors overflow-hidden hover:bg-muted`}>
                  ...
                  <div className={`text-xs mt-1 text-primary/70`}>{r.date}</div>
                </div>
              ))}
            </motion.div>
          )}
        ...
        {isOpen && (
          ...
          >
            ...
            <input
              ...
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              className={`flex-1 bg-transparent border-none py-3 pl-10 pr-4 text-base outline-none w-full placeholder-primary/70 text-foreground italic`}
            />
            <button 
            ...
            {query.length > 0 && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className={`absolute top-full left-0 right-0 mt-0 shadow-2xl flex flex-col p-2 z-50 min-h-[calc(100vh-64px)] ${mobileBgColor} border-t border-border`}
              >
                {results.map((r, i) => (
                  <div key={i} className={`px-4 py-4 rounded-md cursor-pointer transition-colors hover:bg-muted border-b border-border last:border-0`}>
                    ...
                    <div className={`text-xs mt-1.5 text-primary/70`}>{r.date}</div>
                  </div>
                ))}
              </motion.div>
            )}
